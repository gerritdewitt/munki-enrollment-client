...
def main():
    ...
    synonyms = ["--cli","--command-line","--commandline","--console","--verbose"]
    for arg in synonyms:
        if arg in sys.argv:
            COMMAND_LINE_MODE = True
            common.print_info("Command line mode enabled.  Output will be more verbose.")
            break
    # PE action:
    synonyms = ["do-pre-enrollment"]
    for arg in synonyms:
        if arg in sys.argv:
    ...
    synonyms = ["transaction-a"]
    for arg in synonyms:
        if arg in sys.argv:
            should_show_help = False
            transaction_a.do_transaction()
   # Run Munki:
    synonyms = ["run-munki-check"]
    for arg in synonyms:
        if arg in sys.argv:
            should_show_help = False
            munki.managedsoftwareupdate_check_in()
            break
    synonyms = ["run-munki-install"]
    for arg in synonyms:
        if arg in sys.argv:
            should_show_help = False
    ...
    for given_arg in sys.argv:
        if given_arg.find("transaction-b") != -1:
            found_transaction_arg = True
        if given_arg.find("group=") != -1:
            try:
                desired_group = given_arg.split('=')[1]
            except IndexError:
                pass
        if given_arg.find("name=") != -1:
            try:
...
USAGE: enrollment-client.py [--cli] command
